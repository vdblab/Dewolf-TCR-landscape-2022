...
plots_to_make <- list(
  ...
  "Recipient 7, rep2, day 14" = c(org="Mouse", ref_tissue="skin", topn=10, ref_color="forestgreen", f="S4A"),
   "Patient B" = c(org="Human", topn=10, ref_tissue="graft_blood", ref_color="navy", f="S4L"), 
  "Patient C" = c(org="Human", topn=10, ref_tissue="graft_blood", ref_color="navy", f="S4M"), 
  "Patient D" = c(org="Human", topn=10, ref_tissue="graft_blood", ref_color="navy", f="S4N"), 
  "Patient E" = c(org="Human", topn=10, ref_tissue="graft_blood", ref_color="navy", f="S4O"), 
  "Patient F" = c(org="Human", topn=10, ref_tissue="graft_blood", ref_color="navy", f="S4P"),
  "Patient G" = c(org="Human", topn=10, ref_tissue="graft_blood", ref_color="navy", f="S4Q")
)
...
for (i in seq_along(plots_to_make)){
  ...
  thisdata <- make_alluvial_dataset(
    tcr_raw, tcr_sample_metadata=thistcr_sample_metadata, ref_org=thing["org"],
...
ptF_alluvial <- make_alluvial_dataset(tcr_raw, ref_org = "Human", ref_pt = "Patient F", ref_tissue = "ascending_colon", tcr_sample_metadata = tcr_sample_metadata, tissue_metadata = tissue_annotations, topn = 5)
...
ggsave(plot=p_ptF_alluvial_A ,height=5,width=10,dpi=200, 
         filename=file.path(alluvial_figures_path, "S4A_PtF_multicolor.pdf"), 
         useDingbats=FALSE)
...
# ggplot() +   geom_line(data=abundance_summary_data_grouped_summaries, aes(x=clone_rank, color=grp, y=mean_perc_cu, group=grp), alpha=.7) 
...
ggplot(abundance_summary_data, aes(x=clone_rank, y=perc_cu, group=sample_name)) +
  geom_line(alpha=.05) +
...
#filtering out samples not included in the alluvials (donor pt A, kidney, heart, and marrow pt D)
 tcr_raw2 <- tcr_raw %>% filter(sample_name != "PtA_donor_TCRB") %>% filter(sample_name != "PtD_autopsy_marrow_TCRB") %>% filter(sample_name != "PtD_kidney_TCRB") %>% filter(sample_name != "PtD_donor_TCRB") %>% filter(org != "Mouse")  
...
summary_data_for_yuval <-  tcr_raw2 %>% 
  group_by(sample_name) %>% 
  arrange(desc(productive_frequency)) %>% 
  mutate(
    clone_rank = rank(desc(productive_frequency), ties.method = "min"),
