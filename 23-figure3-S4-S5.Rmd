...
abundance_summary_data <-  tcr_raw %>% 
  group_by(sample_name) %>% 
  arrange(desc(productive_frequency)) %>% 
  mutate(clone_rank = rank(desc(productive_frequency), ties.method = "first"),
         cumu_templates = cumsum(templates),) %>%
...
abundance_summary_data_grouped_summaries <- abundance_summary_data %>% 
  ...
  summarize(
    grp=paste0(org, "-", gvhd),
    mean_perc_cu = mean(perc_cu)) %>% distinct() %>% ungroup() 
...
ggplot(abundance_summary_data, aes(x=clone_rank, y=perc_cu, group=sample_name)) +
  geom_line(alpha=.05) +
  geom_smooth(data=abundance_summary_data_grouped_summaries, aes(x=clone_rank, color=grp, y=mean_perc_cu, group=grp), alpha=.7) +
  geom_vline(xintercept = 10, linetype="dotted", color="gray") + scale_x_log10() 
