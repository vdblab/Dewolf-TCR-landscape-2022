...
stat_pvalue_manual(
  ...
  gridExtra::tableGrob(tissue_annotations %>% select(plotgroup,  tissue) %>% distinct() %>% arrange(desc(plotgroup)), 
                       theme = gridExtra::ttheme_minimal( base_size=7),
                       rows=NULL) + 
  plot_layout(widths = c(8, 2)))

ggsave(p_between_within, filename = file.path(boxdir, "mouse_human_between_within.pdf"), width = 16, height = 8)
