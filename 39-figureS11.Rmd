...
p_between_within <- ggplot(thisdata, aes(x=grp, color=grp, group=grp, y=value)) + 
  ...
  scale_color_manual(values = c("navy", "firebrick4"), breaks = c("Between", "Within"), labels=c("Between systems", "Within systems")) + 
...
for (si in 1:length(sets_of_comparisons)){
  ...
  ggsave(p_between_within_intestine + p_shuff_intestinal_summary + plot_layout(ncol=1, heights = c(.8, .3)) , filename = file.path(boxdir, str_glue("mouse_human_between_within_{sname}.pdf")), width = 8, height = 8)
...
#formatting individual graph to focus on comparisons with blood (Figure R1)
  tmp_gvhd <- ggplot(tcr_corrs_anno %>% 
           filter(indv_label.x == indv_label.y) %>%
             ...
             left_join(tissue_annotations, by=c("tissue.y"="tissue")) , aes(x=indv_label.x, y=JSD)) + 
    geom_violin() + 
