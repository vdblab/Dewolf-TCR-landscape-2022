...
make_alluvial_dataset <- function(tcr_raw, tcr_sample_metadata, ref_org="Human", ref_pt="Patient B", ref_tissue="duodenum", topn=10){
  ...
  p_ <- ggplot(plot_data,
                aes(x = reorder(tissue_type, anatomic_order), stratum = frequency_nudged, y=frequency_nudged, alluvium = key,
                    fill = ifelse(key%in%1:10, "mediumpurple", "gray75"))) +
...
thisdata <- make_alluvial_dataset(tcr_raw, tcr_sample_metadata)
thisdata$p_

plots_to_make <- list(
  "Patient A"=c(org="Human", topn=10, ref_tissue="esophagus"),
  "Recipient 7, rep2, day 14"=c(org="Mouse", ref_tissue="skin", topn=10)
)
...
ggsave(plot=thisdata$p_ ,height=5,width=10,dpi=200, 
       filename=paste0("alluvial_", names(plots_to_make)[1], ".pdf"), 
       useDingbats=FALSE)
}
```
