...
file.copy(file.path(boxdir, paste0("between_within_Human.pdf")),
          file.path(boxdir, paste0("S3E-between_within_Human.pdf")))
file.copy(file.path(boxdir, paste0("between_within_Mouse.pdf")),
...
file.copy(file.path(boxdir, str_glue("mouse_human_between_within_{sname}.pdf")),
...
```{r}
p_mouse_cumutop10 <- tcr_raw %>% filter(org=="Mouse") %>% filter(grepl("Balb|Recipient", sample_name)) %>% mutate(harvest_day=ifelse(is.na(collection_day), "Donor", collection_day)) %>% 
  group_by(sample_name) %>% mutate(rank=rank(desc(productive_frequency), ties.method = "min")) %>% 
  ungroup() %>% 
  filter(rank <= 10) %>% 
  mutate(tissue=ifelse(tissue=="terminalileum", "ileum", tissue)) %>% 
  group_by(sample_name, tissue, harvest_day) %>% 
  summarize(sum_prod_freq = sum(productive_frequency)) %>% 
  ggplot(aes(x=harvest_day, fill=harvest_day, y=sum_prod_freq)) + 
  geom_boxplot(outlier.colour = NA) + 
  geom_jitter(height = 0, alpha=.4, size=2) +
  scale_x_discrete(breaks=c("Donor", "7", "14"), limits=c("Donor", "7", "14")) + 
  scale_fill_manual(values=c("Donor"="#CCD2D0", "14"="#B6DFE9", "7"="#172470")) +
  facet_grid(~tissue) +
  theme_bw()
