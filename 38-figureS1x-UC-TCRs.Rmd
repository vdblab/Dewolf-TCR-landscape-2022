...
uc_corrs_raw <- read.csv(sep = "\t", "data/2022-09-06-UC-tcr-correlations.tsv", header=FALSE, col.names = strsplit("fileA\tfileB\tsizeA\tsizeB\tnorm_sizeA\tnorm_sizeB\tthreshold\tJSD\tjsd_nt_norm\tjsd_aa_raw\tjsd_aa_norm\tmorisita_nt_raw\toverlapping_clones\toverlapping_aa", split = "\t")[[1]], na.strings = c("nan"))  %>% filter(JSD != 0) %>% arrange(fileA, fileB)
uc_corrs_filtered1k <- read.csv(sep = "\t", "data/2022-09-21-UC-tcr-correlations.tsv", header=FALSE, col.names = strsplit("fileA\tfileB\tsizeA\tsizeB\tNA\tthreshold\tnorm_sizeA\tnorm_sizeB\tJSD\tjsd_nt_norm\tjsd_aa_raw\tjsd_aa_norm\tmorisita_nt_raw\toverlapping_clones\toverlapping_aa", split = "\t")[[1]], na.strings = c("nan")) %>% filter(JSD != 0) %>%  arrange(fileA, fileB)
tmpdf <- data.frame(raw=uc_corrs_raw$JSD, filtered1k=uc_corrs_filtered1k$JSD)
tmpdf$raw - tmpdf$filtered1k
ggplot(tmpdf, aes(x=raw, y=filtered1k) ) + geom_point()



thismetric = "JSD"
msk_uc_corrs <- read.csv(sep = "\t", "data/2022-09-21-UC-tcr-correlations.tsv", header=FALSE, col.names = strsplit("fileA\tfileB\tsizeA\tsizeB\tnorm_sizeA\tnorm_sizeB\tthreshold\tJSD\tjsd_nt_norm\tjsd_aa_raw\tjsd_aa_norm\tmorisita_nt_raw\toverlapping_clones\toverlapping_aa", split = "\t")[[1]], na.strings = c("nan")) %>%
