...
div_base <- ggplot(diversity %>% filter(org == "Human"), aes(y=reorder(tissue_label, desc(anatomic_order))))
...
ggsave(plot=fig1c ,height=5,width=6, filename="fig1c.pdf")
...
ggsave(patchwork::plot_spacer()  + fig1b + fig1c + fig1d + fig1e + plot_layout(widths = c(1, .7, 1,1,1), nrow=1, guides="collect") + plot_annotation(tag_levels = "A"), width = 24, height=5, filename = "fig1.pdf")
```
...
```{r}
div_base_mouse <- ggplot(diversity %>% filter(org != "Human"), aes(y=reorder(tissue_label, desc(anatomic_order))))
