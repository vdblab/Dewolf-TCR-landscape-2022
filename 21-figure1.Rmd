...
div_base <- ggplot(diversity %>% filter(sample_name != "PtD_autopsy_marrow_TCRB") %>% filter(org == "Human"), aes(y=reorder(tissue_label_fig1, desc(anatomic_order)))) 
...
(fig1f <- div_base +
    geom_point(aes(x=productive_templates, fill = indv_label), size = 4, shape=21, alpha = .9, position = jitter) +
    theme_classic()  +
    scale_x_log10(
      breaks = c(1 %o% 10^(-6:6)),
      labels = scales::trans_format("log10", scales::math_format(10^.x))
...
ggsave(plot=fig1f ,height=5,width=6, filename=file.path(figures_path, "fig1f.pdf"))
