...
write.csv(tcr_corrs_anno   %>% filter(grepl("^Pt", sample1) & grepl("^Pt", sample2)) %>% 
            ...
            select(sample1, sample2, JSD, jsd_aa_raw, morisita_nt_raw) %>% 
            rename(
              `JSD (nucleotide)`=JSD,
              `JSD (amino acid)` = jsd_aa_raw, 
                   `Morisita (nucleotide)`= morisita_nt_raw) %>% 
             mutate_if(is.numeric, thisround), row.names = FALSE, file = file.path(tab_dir, "st6-human-tcr-comparisons.csv"))
...
write.csv(tcr_corrs_anno   %>% filter(!grepl("^Pt", sample1) & !grepl("^Pt", sample2)) %>% 
            ...
            rename(
              ...
              `JSD (amino acid)` = jsd_aa_raw, 
                   `Morisita (nucleotide)`= morisita_nt_raw) %>% 
             mutate_if(is.numeric, thisround), row.names = FALSE, file = file.path(tab_dir, "st11-mouse-tcr-comparisons.csv"))
