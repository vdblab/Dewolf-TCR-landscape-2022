...
tcr_topn_filtered <- tcr_raw %>% 
  ...
  group_by(sample_name) %>% 
    slice_max(order_by = productive_frequency, n = threshold) %>%
  ...
  results[[pt]] <- tcr_topn_filtered_jsd
}
...
tcr_depth_filtered <- tcr_raw %>% 
    ...
    filter(!is.na(productive_frequency)) %>% 
  mutate(lowdepth_templates = (productive_templates *depth_threshold) / productive_rearrangements) %>%
