...
if(!file.exists(tcr_objects_path)){
  ...
  tcr_sample_metadata <- tcr_raw %>% 
    select(sample_name, org,  samp_type, collection_day,  sample_amount_ng, indv_type, exp_rep, gvhd, indv_id, indv_label, tissue) %>% 
    distinct()
  ...
  new_tcr_corrs <- read.csv(sep = "\t", "2022-08-29-tcr-correlations.tsv", header=FALSE, col.names = strsplit("fileA\tfileB\tsizeA\tsizeB\tnorm_sizeA\tnorm_sizeB\tjsd_nt_raw\tjsd_nt_norm\tjsd_aa_raw\tjsd_aa_norm\tmorisita_nt_raw\toverlapping_clone", split = "\t")[[1]], na.strings = c("nan"))
  #do a double join to get all (non-unique combinations)
  tmp <- rbind(
    new_tcr_corrs, 
    new_tcr_corrs %>% 
      mutate(sizeX=sizeA, sizeA=sizeB, sizeA=sizeX, norm_sizeX=norm_sizeA, norm_sizeA=norm_sizeB, norm_sizeA=norm_sizeX) %>% 
      select(-sizeX, -norm_sizeX)
    ) %>%
    bind_rows(new_tcr_corrs %>% select(fileA, sizeA) %>% distinct() %>% mutate(fileB=fileA, sizeB=sizeA))
