...
depth_data <- tcr_corrs %>% 
  ...
  left_join(total_clones, by=c("sample1"="sample1")) %>%
  left_join(total_clones, by=c("sample2"="sample1")) %>% 
  ...
  mutate(
    pct_overlap = pmin(total_clones.x, total_clones.y)/pmax(total_clones.x, total_clones.y),
    pair_max_clones = pmax(total_clones.x, total_clones.y),
    pair_min_clones = pmin(total_clones.x, total_clones.y),
         )%>%
  rowwise() %>% 
  mutate(
    gmean = sqrt(total_clones.x*total_clones.y),
    mean = mean(c(total_clones.x, total_clones.y)),
  )

ggplot( depth_data,
          aes(x=total_clones.x, y=total_clones.y)) + 
...
#human
  ggplot( depth_data %>% filter(org.x == "Human"),
          aes(x=pct_overlap, y=JSD)) + 
  geom_point(aes(color=pair_min_clones), alpha=.3) + 
  scale_color_gradient(low="blue", high = "red") + 
...
for (i in 1:nrow(tcr_corrs_equalized)){
  ...
  depth_threshold <- min(tcr_corrs_equalized$total_clones.x[i], tcr_corrs_equalized$total_clones.y[i] )
  tcr_depth_filtered <- tcr_raw %>% 
...
```{r}
for (pt in unique(tcr_corrs_equalized %>% filter(org.x == "Human") %>% pull(indv_label.x))){
