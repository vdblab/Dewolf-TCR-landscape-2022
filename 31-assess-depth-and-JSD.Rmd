...
---
```{r 31-assess-depth-and-JSD-1, include=FALSE}
...
for (i in 1:nrow(tcr_corrs_equalized)){
  ...
  tcr_depth_filtered <- tmp %>% data.table::as.data.table() %>% 
  ...
  if (nrow( tmp[tmp$sample_name == tcr_corrs_equalized$sample1[i], ]) > 0){
    tcr_corrs_equalized$sample1_total_lowdepth_tempates[i] <- tmp[tmp$sample_name == tcr_corrs_equalized$sample1[i], ] %>% pull(total_lowdepth_templates) %>% unique()
  } else {
    tcr_corrs_equalized$sample1_total_lowdepth_tempates[i] = 0
  }
  if (nrow( tmp[tmp$sample_name == tcr_corrs_equalized$sample2[i], ]) > 0){
    tcr_corrs_equalized$sample2_total_lowdepth_tempates[i] <- tmp[tmp$sample_name == tcr_corrs_equalized$sample2[i], ] %>% pull(total_lowdepth_templates) %>% unique()
  } else {
    tcr_corrs_equalized$sample2_total_lowdepth_tempates[i] = 0
  }
  
}

ggplot(tcr_corrs_equalized %>% 
         mutate(per_remaining_of_min = (pmin(sample1_total_lowdepth_tempates, sample2_total_lowdepth_tempates))/pair_min_templates), aes(x=JSD, y=JSD_norm, color=per_remaining_of_min)) + 
  geom_point() +
...
ggplot( tcr_corrs_equalized,
        aes(x=gmean, y=JSD_norm)) + 
  geom_point(aes(color=JSD_norm), alpha=.3) + 
  scale_color_gradient(low="blue", high = "red") + 
  scale_y_log10() + scale_x_log10() + 
  geom_smooth(method="lm") + 
  facet_grid(~org.x) 
...
pt_a_jsd_gmean<- plot_patient_tcr_tree(df = tcr_corrs_equalized %>%ungroup() %>%
                        filter(!is.na(JSD_norm)) %>% 
                        filter(indv_label.x == pt) %>% 
                        left_join(tcr_sample_metadata, by=c("sample1"="sample_name")) %>% 
                        left_join(tcr_sample_metadata, by=c("sample2"="sample_name")),
                        metri = "gmean", patient = pt)
