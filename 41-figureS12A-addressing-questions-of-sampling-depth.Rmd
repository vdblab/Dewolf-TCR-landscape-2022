...
library(preseqR)
# load("../Dewolf-TCR-landscape-2022/tcr_objects.Rdata")

tcr_counts_gvh <- tcr_raw %>% select(sample_name, rearrangement,templates )
tcr_counts_gvh %>% glimpse()
tcr_counts_gvh %>% head()
tcr_counts_gvh %>%
  count(rearrangement) %>% nrow()
duplicated(tcr_counts_gvh$rearrangement) %>% table()
tcr_counts_gvh %>% count(sample_name) %>% print(n = 400)
...
tcr_hist %>% glimpse()
# dat <- tcr_hist %>%filter(sample_name == "Balb-c_1_blood")
```
...
p_preseq_extrap_by_tissue <-  ggplot(tcr_increase_results %>% group_by(sample_name) %>%
...
tcr_preseq_increase <- tcr_increase_results %>% group_by(sample_name) %>%
  summarize(perc_increase=max(yield.estimate)/min(yield.estimate)) %>%
  left_join(tcr_sample_metadata %>% select(sample_name, indv_label, tissue, org, gvhd)) %>%
  ...
  group_by(gr, org) %>% do(x=broom::tidy(summary(.$perc_increase))) %>% unnest(x) %>%
  arrange(org, gr) %>% mutate(across(.cols = minimum:maximum, round, 2)) %>% rename(`Tissue Type`=gr, `Species`=org)
...
tmp <- tmp_long %>% pivot_wider(names_from = sample_name, values_from=templates, values_fill = 0) %>%
...
tmpdf <- map(seq(1,  max(colSums(tmp)), 10), .f = function(x){
  data.frame(Rearrangements=vegan::rarefy(t(tmp), sample = x, se = FALSE, )) %>% rownames_to_column("sample_name") %>% mutate(`Sample Size` = x) 
}) %>% 
  bind_rows() %>% 
...
p_rare <- ggplot(tmpdf, aes(x=`Sample Size`, y=Rearrangements, color=sample_name)) +
  ...
  geom_text(data=these_preseq %>% group_by(sample_name) %>% mutate(lab=ifelse(sample.size == max(sample.size), sample_name, "")), 
...
ggsave(p_rare,
  filename = paste("exemplar_rarefaction_curves-", paste0(names(tmp), collapse="__"), ".pdf"), width = 8, height=8
  # p_rare + inset_element(p_rare_inset, left = .1, bottom = 0.7, right = .5, top = 1)
)
ggsave(p_rare,
  filename = paste("exemplar_rarefaction_curves-", paste0(names(tmp), collapse="__"), ".png"), width = 8, height=8
  # p_rare + inset_element(p_rare_inset, left = .1, bottom = 0.7, right = .5, top = 1)
)


tt2 <- gridExtra::ttheme_minimal(core=list(fg_params=list(hjust=1, x=0.9)),
...
ggsave(combined_fold_increase_plot,
  filename = paste("SX-exemplar_rarefaction_curves-", paste0(names(tmp), collapse="__"), ".png"), width = 7, height=9
)
ggsave(combined_fold_increase_plot,
  filename = paste("SX-exemplar_rarefaction_curves-", paste0(names(tmp), collapse="__"), ".pdf"), width = 7, height=9
)
```    
