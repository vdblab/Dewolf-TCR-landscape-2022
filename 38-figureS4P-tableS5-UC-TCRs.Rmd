...
table_sx <- msk_overlapping_bioidentities %>% filter(gvhd=="GvHD") %>% select(sample_name, v_gene, j_gene, cdr3_amino_acid, n_samples, relab) %>%   pivot_wider(id_cols = c(v_gene,  cdr3_amino_acid, j_gene, n_samples), names_from=sample_name, values_from=relab, values_fill = 0)
 
write.csv(table_sx, "tables/st7-uc-clones-in-gvhd-samples.csv", row.names = FALSE)
...
ggplot(left_join(msk_overlapping_bioidentities, clonotypes, by=c("cdr3_amino_acid"="CDR3.aa")), 
#       aes(x=relab, y= paste0(Pathology, "\n", Antigen.protein), size=n_samples, fill=Category )) +
       aes(x=relab, y=str_wrap(Pathology), size=n_samples, fill=Category )) +
  geom_point(shape=21) + 
  facet_grid(Category  ~ gvhd, space="free", scales = "free_y")
