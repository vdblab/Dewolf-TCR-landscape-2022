...
# by sample
ggplot(uc_msk_joined_clonotype_data, 
...
ggplot(uc_msk_joined_clonotype_data_by_pt, 
...
clonotype_data <- uc_msk_joined_clonotype_data  %>% 
         ...
         group_by(tissue_label_fig1, indv_label, sample_name, cohort, Pathology, Category) %>% 
         summarize(n_clonotypes =n_distinct(cdr3_amino_acid),
                   max_cdr3_relab = max(total_cdr3_productive_frequency),
                   median_rank = median(per_sample_rank)) %>% 
         ungroup() %>% 
         filter(!is.na(Pathology) & !is.na(Category)) %>% 
         group_by(Pathology) %>%
         mutate(yorder=median(n_clonotypes)) %>% 
           ...
           mutate(relab_abundance_cat = case_when(
             max_cdr3_relab >= .05 ~ ">= .05",
             max_cdr3_relab >= .01 ~ "(.05 - .01]",
             max_cdr3_relab >= .005 ~ "(.01 - .005]",
             TRUE ~ "< .005",
             ))
p_clonotyes_by_tissue_heatmap_by_rank <- ggplot(clonotype_data %>% filter(Pathology != "Unknown", Pathology != "Encounter for immunization") , 
       aes(x=tissue_label_fig1, y=reorder(str_wrap(Pathology, 45), yorder), color = reorder(relab_abundance_cat, max_cdr3_relab))) +
  geom_point() + 
  ...
  labs(x="Tissue", y="Associated Pathology Category \n (Pagliuca, et al. 2021)", color="Maximum Relative\nAbundance")
ggsave(p_clonotyes_by_tissue_heatmap_by_rank, filename = "figures/UC/Revision_clonotypes_by_tissue_heatmap_by_rank.pdf",
       width=10, height = 7)
write.csv(clonotype_data, "tables/st5-annotated-clones.csv", row.names = FALSE)
